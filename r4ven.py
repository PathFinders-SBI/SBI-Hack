...
def get_user_choice():
    """Prompt the user for which type of data to collect."""
    ...
    print(f"\n{M}Note: {W}IP address & Device details are always collected regardless of choice.")
    choice = input(f"\n{B}[+] {Y}Enter the number corresponding to your choice: {W}")
    return choice

def main():
    """Main runner function for R4VEN."""
    print_banners()

    log_file_path = os.path.abspath(log_file)
    print(f"{B}[+] {Y}Logs :{W} {log_file_path}\n")

    # ✅ Check if chosen port is free
    ...
    # ✅ Ask user what they want to collect
    ...
    # ✅ Ask which tunneling method to use
